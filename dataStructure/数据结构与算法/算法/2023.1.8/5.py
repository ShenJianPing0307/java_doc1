...
def mergeWrap(
        left_and_right_arr):  # Como le pasamos una tupla de arrays, asignamos cada uno a su variable correspondiente y hacemos el merge()
...
def mergeSortParallel(array_to_sort):
    ...
    pool = Pool(
        processes=n_cores)  # Establecemos el numero de procesos creados/utilizados como el numero de cores, por lo tanto paralelismo
    ...
    while len(
            sorted_sublists) > 1:  # Mientras haya mas de 1 chunk, significa que todavia no hemos terminado de merge todos los chunks
        ...
        sorted_sublists = pool.map(mergeWrap,
                                   chk_lst)  # Paralelizamos tambien la operacion mergeWrap(), mirar comentario de la funcion

    return sorted_sublists[
        0]  # Todos los chunks se han combinado en una sola lista, por lo que en el primer y unico indice tenemos el resultado


if __name__ == "__main__":
    print("Generando array con 21,802,766 elementos...")
    inicio_garray = time.time()
    array_to_sort = [random.randint(0, 30_000) for i in
                     range(21802766)]  # Generamos 21_802_766 elementos que varian desde el 0 al 30_000 inclusive
